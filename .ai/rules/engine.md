...
**Use `2-mirrors-oposite` to test anything about tunnels** — Paul built it and it is what made this findable: a facing pair is a single chain going straight out, with no branching, no commuting pairs, and no opening clipped by a third surface, and a wrap-around portal (`new-level`'s `one`) is the same shape by a different mechanism. Four mirrors give 3^n chains and every confound at once; two give one chain n levels deep and a fault you can attribute.
...
## A counting walk must draw nothing, including the bookkeeping
The walks that choose the frame's floor and depth return as soon as they have counted a chain. They used to fall through into the showing loop, which sets every pane's visibility, points each one's material at a render target, and computes the affine read for it — all for a walk that draws nothing at all.
