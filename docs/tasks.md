...
Last audited against the repo at `da5a5ce`.
...
**`main` at `da5a5ce` is green, verified here.** The whole gate, run against `main` rather
than against a branch: eslint, prettier, tsc, pint and phpstan all pass, **41 component tests
...
| 1-14 | Stair generator in the editor | **next** | — | **The user's choice for this lane.** ~2 days. Select a room, say how many steps over what rise, and the editor carves it into N sectors with heights already set; `carve.ts` already splits a room into slabs via `intoSlabs`. Pairs with the slopes that landed tonight. **Do not bake a `MAX_STEP` assumption into the geometry** — agent A's vertical-physics work makes that a runtime decision, and `MAX_STEP` is pinned in two lanes at once. See ISSUE-45. |
