...
Last audited against the repo at `89fe501`.
...
**`main` at `89fe501` is green, verified here.** The whole gate, run against `main` rather
than against a branch: eslint, prettier, tsc, pint and phpstan all pass, **43 component tests
in 5 files** under Vitest, Pest **355 of 355, 2436 assertions**.
...
| 1-14 | Stair generator in the editor | **wip — start here** | — | **The user's choice for this lane.** ~2 days. Select a room, say how many steps over what rise, and the editor carves it into N sectors with heights already set; `carve.ts` already splits a room into slabs via `intoSlabs`. Pairs with the slopes that landed tonight. **Do not bake a `MAX_STEP` assumption into the geometry** — agent A's vertical-physics work makes that a runtime decision, and `MAX_STEP` is pinned in two lanes at once. See ISSUE-45. |
