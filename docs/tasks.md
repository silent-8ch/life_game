...
Last audited against the repo at `bd0c9bc`.
...
**`main` at `bd0c9bc` is green, verified here.** The whole gate, run against `main` rather
than against a branch: eslint, prettier, tsc, pint and phpstan all pass, **41 component tests
in 5 files** under Vitest, Pest **345 of 345, 2407 assertions**.
...
| ID | Task | Status | Depends on | Notes |
| --- | --- | --- | --- | --- |
| A-03 | Prop rendering — engine | **blocked on art, not on code** | ART-01 | Render modes `box`/`billboard`/`cross`, fitted UVs, cutout with `alphaTest`, prop texture loader path, frame animation, alt-state by flag. **Billboards must face whichever camera is drawing** — same trap as the sky dome. Plan: `plan-prop-rendering.md`. |
...
| ISSUE-11 | **Files over 1100 lines.** The two lines have crossed. `level-viewport.tsx` is **928** and falling — two of five cuts in. `inspector.tsx` is **1398** and rising: 1-04 added 148 and 1-06 another 135, both as planned. It is now **the largest file this project has ever had**, past the 1242 `build-level.ts` stood at when A-01 split it. That was the right call each time — the panels are conditional and self-contained, and neither task was cheaper after a split than before — but it is the whole argument for 1-08 not slipping again. | medium | 1 / A | partly closed |
