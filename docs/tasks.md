...
Last audited against the repo at `7e7f529`.
...
not need agent 1 first. **A-04 must not start before A-09's `player.ts` cut has landed — see ISSUE-26.** They touch the
same lines and A-04 is only one agent-1 task away.
...
**`main` at `7e7f529` is green, verified here.** The whole gate, run against `main` rather
than against a branch: eslint, prettier, tsc, pint and phpstan all pass, Pest **328 of 328,
2365 assertions, 37s**.
