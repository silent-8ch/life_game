...
| 1-12 | Split `inspector.tsx` | **go — start here** | ~~1-11~~ in | ISSUE-27's JSX half, and the coverage it was waiting for exists: 41 tests that pin the boundaries between the panels, which is exactly what a move can break. Agent 1, who wrote them, judges them adequate for the move and that is the judgement that counts. Level, room and wall want to be three panels rather than one 500-line block — that is where the size actually is. Run the component suite between each panel moved, not once at the end. |
| 1-13 | Three accessibility gaps in the editor | **needs the user** | — | Found by 1-11, none fixed, because all three change the product rather than the tests. `TexturePicker` renders its label as a bare `<span>` with nothing tying it to the control, so a screen reader announces an unlabelled button. `Toggle` is a plain `<button>` with no `aria-pressed`, so its state is invisible to assistive technology. Both are why the tests query by text and assert through the callback rather than using `getByLabelText` and `toBeChecked` — the comments say so, so the query style is not read as sloppiness. Small fixes, real improvement. |
