...
Last audited against the repo at `330ec27`.
...
**`main` at `330ec27` is green, verified here.** The whole gate, run against `main` rather
than against a branch: eslint, prettier, tsc, pint and phpstan all pass, Pest **338 of 338,
2386 assertions, 40s**.
...
| 1-06 | Slopes — editor | **wip** | 1-05 | Inspector hinge picker and rise field; side view drawing the section slanted. Grows `inspector.tsx` again before 1-08 shrinks it — eyes open, per the ordering. |
...
| 1-08 | React component test runner | todo — **now also gates ISSUE-27** | — | **Decided: Vitest + Testing Library** (ISSUE-6). The inspector split cannot be evidenced without it, so this is no longer only a coverage task. Adds two dev dependencies — that approval is on the record here, so take it rather than re-asking. Wire it into `composer ci:check` alongside `types:check`, and land one real test on `inspector.tsx` in the same branch so the harness is proven, not just installed. Say so here before you touch `package.json`. |
...
| ISSUE-11 | **Files over 1100 lines.** `build-level.ts` came off at A-01. `inspector.tsx` is **1302** now — 1-04 added 148 as expected, and 1-06 is still to come. `level-viewport.tsx` is **1084**: A-02 took it to 1017 and the scan wiring put 67 back. Both are mid-plan rather than drifting — A-09 is cutting the viewport now, and ISSUE-27 has the inspector behind 1-08 — but the numbers are going the wrong way in the meantime and that is worth seeing rather than assuming. | medium | 1 / A | partly closed |
